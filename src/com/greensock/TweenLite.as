   ...
   public class TweenLite extends TweenCore
   {
      ...
      public static function killTweensOf(param1:Object, param2:Boolean = false, param3:Object = null) : void
      {
         if(param1 in masterList)
         {
            ...
            while(true)
            {
               ...
               var _loc6_:TweenLite = null[0];
               if(!_loc6_.gc)
               {
                  if(param2)
                  {
                     null.complete(false,false);
                  }
                  if(param3 != null)
                  {
                     null.killVars(param3);
                  }
                  if(param3 == null || null.cachedPT1 == null && Boolean(null.initted))
                  {
                     null.setEnabled(false,false);
                  }
               }
            }
            if(param3 == null)
      ...
      public function killVars(param1:Object, param2:Boolean = true) : Boolean
      {
         ...
         for(_loc3_ in param1)
         {
            ...
            {
               var _loc4_:PropTween = propTweenLookup[_loc3_];
               if(_loc4_.isPlugin && null.name == "_MULTIPLE_")
               {
                  null.target.killProps(param1);
                  if(null.target.overwriteProps.length == 0)
                  {
                     null.name = "";
                  }
                  if(_loc3_ != null.target.propName || null.name == "")
                  {
                     delete propTweenLookup[_loc3_];
                  }
               }
               if(null.name != "_MULTIPLE_")
               {
                  if(null.nextNode)
                  {
                     null.nextNode.prevNode = null.prevNode;
                  }
                  if(null.prevNode)
                  {
                     null.prevNode.nextNode = null.nextNode;
                  }
                  else if(this.cachedPT1 == null)
                  {
                     this.cachedPT1 = null.nextNode;
                  }
                  if(Boolean(null.isPlugin) && Boolean(null.target.onDisable))
                  {
                     null.target.onDisable();
                     if(null.target.activeDisable)
                     {
