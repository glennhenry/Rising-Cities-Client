package org.bytearray.explorer
{
   ...
   public final class SWFExplorer extends URLLoader
   {
      ...
      public function parse(param1:ByteArray, param2:String = "classes") : Array
      {
         ...
         var _loc6_:uint = uint(_loc3_ & 7);
         this.source = this.swf.readUnsignedByte();
         var _loc7_:uint = uint(this.readBits(_loc5_) | (_loc6_ << _loc5_) / 20);
         var _loc8_:uint = this.readBits(_loc4_) / 20;
         var _loc9_:uint = this.readBits(_loc4_) / 20;
         var _loc10_:uint = this.readBits(_loc4_) / 20;
         var _loc11_:uint = uint(this.swf.readShort() & 0xFF);
         var _loc12_:uint = uint(this.swf.readShort());
         var _loc13_:uint = uint(_loc12_ >> 8 & 0xFF | (_loc12_ & 0xFF) << 8);
         ...
         if(param2 == SWFExplorer.CLASSES)
         {
            ...
            if(_loc14_.length > 0)
            {
               ...
               _loc16_ = int(this.swf.readUnsignedShort());
               _loc15_ = 0;
               while(_loc15_ < _loc16_)
               {
                  this.swf.readUnsignedShort();
                  _loc17_ = this.swf.readByte();
                  _loc18_ = new String();
                  while(_loc17_ != 0)
                  {
                     _loc18_ += String.fromCharCode(_loc17_);
                     _loc17_ = this.swf.readByte();
                  }
                  this.arrayClasses.push(_loc18_);
                  _loc15_++;
               }
            }
            else
