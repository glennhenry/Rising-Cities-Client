package org.puremvc.as3.core
{
   ...
   public class View implements IView
   {
      ...
      public function notifyObservers(param1:INotification) : void
      {
         ...
         {
            var _loc2_:Array = observerMap[param1.getName()] as Array;
            var _loc3_:Array = new Array();
            _loc5_ = 0;
            while(_loc5_ < null.length)
            {
               var _loc4_:IObserver = null[_loc5_] as IObserver;
               null.push(null);
               _loc5_++;
            }
            _loc5_ = 0;
            while(_loc5_ < null.length)
            {
               _loc4_ = null[_loc5_] as IObserver;
      ...
      public function registerMediator(param1:IMediator) : void
      {
         ...
         if(_loc2_.length > 0)
         {
            var _loc3_:Observer = new Observer(param1.handleNotification,param1);
            _loc4_ = 0;
            while(_loc4_ < _loc2_.length)
            {
               registerObserver(_loc2_[_loc4_],null);
               _loc4_++;
            }
         }
         param1.onRegister();
